
data modify entity @s transformation set value [1f,0f,0f,0f,0f,1f,0f,0f,0f,0f,1f,0f,0f,0f,0f,1f]

scoreboard players set $m.m00 gm4_ghef_data 1000
scoreboard players set $m.m01 gm4_ghef_data 0
scoreboard players set $m.m02 gm4_ghef_data 0

scoreboard players set $m.m10 gm4_ghef_data 0
scoreboard players set $m.m11 gm4_ghef_data 1000
scoreboard players set $m.m12 gm4_ghef_data 0

scoreboard players set $m.m20 gm4_ghef_data 0
scoreboard players set $m.m21 gm4_ghef_data 0
scoreboard players set $m.m22 gm4_ghef_data 1000